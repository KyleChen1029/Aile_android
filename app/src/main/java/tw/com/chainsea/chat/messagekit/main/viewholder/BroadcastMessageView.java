...
import tw.com.chainsea.chat.databinding.MsgkitBroadcastBinding;
 ...
 * @date 2020-08-19
 */
public class BroadcastMessageView extends MessageBubbleView<BroadcastContent> {
    final static int SMALL_PIC_SIZE = 460;

    private final MsgkitBroadcastBinding binding;

    public BroadcastMessageView(@NonNull ViewBinding binding) {
        super(binding);
        this.binding = MsgkitBroadcastBinding.inflate(LayoutInflater.from(itemView.getContext()), null, false);
...
//            itemView.setAlpha(1.0f);
            ...
            int displayWidth = UiHelper.getDisplayWidth(binding.getRoot().getContext());
            ...
            binding.rvTopicList.setLayoutManager(new LinearLayoutManager(binding.rvTopicList.getContext(), LinearLayoutManager.HORIZONTAL, false) {
                @Override
    ...
    private View assemblyTextView(IMessageContent iMessageContent) {
        ...
        View child = LayoutInflater.from(binding.getRoot().getContext()).inflate(R.layout.msgkit_text, binding.flContent, false);
    ...
    private View assemblyImageView(ImageContent imageContent) {
        View child = LayoutInflater.from(binding.getRoot().getContext()).inflate(R.layout.msgkit_image, binding.flContent, false);
        ...
        PhotoService.post(image.getContext(), url, image, isGreenTheme ? R.drawable.image_load_error_green : R.drawable.image_load_error, new ProgressServiceCallBack<>() {
            @Override
            ...
            public void error(String message) {
                ...
                image.setImageResource(isGreenTheme ? R.drawable.image_load_error_green : R.drawable.image_load_error);
            }
        ...
        // 順便把原圖載進 Cache
        PhotoService.post(image.getContext(), imageContent.getUrl(), image, isGreenTheme ? R.drawable.image_load_error_green : R.drawable.image_load_error, new ProgressServiceCallBack<>() {
            @Override
            public void progress(float progress, long total) {
    ...
    private View assemblyStickerView(StickerContent stickerContent) {
        View child = LayoutInflater.from(binding.getRoot().getContext()).inflate(R.layout.msgkit_sticker, binding.flContent, false);
...
//        }
    ...
    public View assemblyFileView(FileContent fileContent) {
        View child = LayoutInflater.from(binding.getRoot().getContext()).inflate(R.layout.msgkit_file, binding.flContent, false);
        CircleImageView iconImage = child.findViewById(R.id.fileIconCIV);
    ...
    public View assemblyVideoView(VideoContent videoContent) {
        View child = LayoutInflater.from(binding.getRoot().getContext()).inflate(R.layout.msgkit_video, binding.flContent, false);
        ...
        PhotoService.loadVideoThumbnail(binding.getRoot().getContext(), url, R.drawable.file_msg_down_bg, new ServiceCallBack<>() {
            @Override
    ...
    public View assemblyVoiceView(VoiceContent voiceContent) {
        View child = LayoutInflater.from(binding.getRoot().getContext()).inflate(R.layout.msgkit_voice, binding.flContent, false);
    ...
    public View assemblyUnDefView() {
        View child = LayoutInflater.from(binding.getRoot().getContext()).inflate(R.layout.item_msg_tip, binding.flContent, false);
