        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (mViewCache != null) {
                        if (mViewCache != this) {
                            ...
                            iosInterceptFlag = isIos;//add by 2016 09 11 ，IOS模式开启的話，且当前有侧滑菜单的View，且不是自己的，就该拦截事件咯。
                        }
                        //只要有一个侧滑菜单处于打开状态， 就不给外层布局上下滑动了
                ...
                case MotionEvent.ACTION_MOVE:
                    //add by 2016 09 11 ，IOS模式开启的話，且当前有侧滑菜单的View，且不是自己的，就该拦截事件咯。滑动也不该出现
                    if (iosInterceptFlag) {
                        break;
                    }
...
/*        mScroller.startScroll(getScrollX(), 0, -getScrollX(), 0);
    ...
    // 2 侧滑删除后自己后，这个View被Recycler回收，复用，下一个進入屏幕的View的状态应该是普通状态，而不是展开状态。
    @Override
