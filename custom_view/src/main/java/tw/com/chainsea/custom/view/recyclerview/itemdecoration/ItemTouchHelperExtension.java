...
//            final ItemTouchHelperExtension.RecoverAnimation anim = mRecoverAnimations.get(i);
//            final int count = c.save();
//            Callback.sUICallback.onDrawOver(c, parent, anim.mViewHolder.itemView, anim.mX, anim.mY, anim.mActionState,
//                false);
//            c.restoreToCount(count);
//        }
//        if (mSelected != null) {
//            final int count = c.save();
//            Callback.sUICallback.onDrawOver(c, parent, mSelected.itemView, dx, dy, mActionState, true);
//            c.restoreToCount(count);
//        }
...
//            anim.update();
//            final int count = c.save();
//            Callback.sUICallback.onDraw(c, parent, anim.mViewHolder.itemView, anim.mX, anim.mY, anim.mActionState,
//                false);
//            c.restoreToCount(count);
//        }
//        if (mSelected != null) {
//            final int count = c.save();
//            Callback.sUICallback.onDraw(c, parent, mSelected.itemView, dx, dy, mActionState,
//                true);
//            c.restoreToCount(count);
//        }
    }
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                switch (swipeDir) {
                    ...
                    case START:
                        targetTranslateY = 0;
                        targetTranslateX = Math.signum(mDx) * getRightSwipeWidth();
                        break;
                    case RIGHT:
                    case END:
                        targetTranslateY = 0;
                        targetTranslateX = Math.signum(mDx) * getLeftSwipeWidth();
                        break;
                    case UP:
                    case DOWN:
                        targetTranslateX = 0;
...
//    }
    ...
    private float getLeftSwipeWidth() {
        if (mSelected instanceof Extension) {
            return ((Extension) mSelected).getLeftMenuActionWidth();
        }
        return mRecyclerView.getWidth();
    }

    private float getRightSwipeWidth() {
        if (mSelected instanceof Extension) {
            return ((Extension) mSelected).getRightMenuActionWidth();
        }
        return mRecyclerView.getWidth();
    }
    ...
    private boolean scrollIfNecessary() {
        ...
        if (scrollX != 0) {
            scrollX = mCallback.interpolateOutOfBoundsScroll(mRecyclerView,
                mSelected.itemView.getWidth(), scrollX, scrollDuration);
        }
        if (scrollY != 0) {
            scrollY = mCallback.interpolateOutOfBoundsScroll(mRecyclerView,
                mSelected.itemView.getHeight(), scrollY, scrollDuration);
        }
    ...
    private void moveIfNecessary(ViewHolder viewHolder) {
        if (mActionState == ACTION_STATE_DRAG && !mRecyclerView.isLayoutRequested()) {
            ...
            if (mCallback.onMove(mRecyclerView, viewHolder, target)) {
                mCallback.onMoved(mRecyclerView, viewHolder,
                    target, toPosition, x, y); // keep target visibleR
            }
        }
    }
    ...
    private ViewHolder findSwipedView(MotionEvent motionEvent) {
        ...
        if ((absDx < mSlop && absDy < mSlop) || (absDx > absDy && lm.canScrollHorizontally()) || (absDy > absDx && lm.canScrollVertically()) || child == null) {
            return null;
        }
    ...
    private boolean checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        ...
        if ((mSelected != null || action != MotionEvent.ACTION_MOVE
            || mActionState == ACTION_STATE_DRAG || !mCallback.isItemViewSwipeEnabled()) || (mRecyclerView.getScrollState() == RecyclerView.SCROLL_STATE_DRAGGING)
            || (vh == null) || swipeFlags == 0) {
            return false;
        }
    ...
    private View findChildView(MotionEvent event) {
        ...
        if (mSelected != null) {
            ...
            if (
                x >= mSelectedStartX + mDx &&
                    x <= mSelectedStartX + mDx + selectedView.getWidth() &&
                    y >= mSelectedStartY + mDy &&
                    y <= mSelectedStartY + mDy + selectedView.getHeight()
            ) {
                return selectedView;
            }
        }
        for (int i = mRecoverAnimations.size() - 1; i >= 0; i--) {
            ...
            if (
                x >= anim.getHitX() &&
                    x <= anim.getHitX() + view.getWidth() &&
                    y >= anim.getHitY() &&
                    y <= anim.getHitY() + view.getHeight()
            ) {
                return view;
            }
        }
    ...
    private int checkHorizontalSwipe(ViewHolder viewHolder, int flags) {
        if ((flags & (LEFT | RIGHT)) != 0) {
            ...
            final float threshold = dirFlag == RIGHT ? getRightSwipeWidth() : getLeftSwipeWidth() * mCallback.getSwipeThreshold(viewHolder);
//                    : getLeftSwipeWidth() * mCallback.getSwipeThreshold(viewHolder);

//            final float threshold =  getSwipeWidth() * mCallback.getSwipeThreshold(viewHolder);
    ...
    public abstract static class Callback {
         ...
         * {@code target}.
         * @see #onMoved(RecyclerView, RecyclerView.ViewHolder, RecyclerView.ViewHolder, int, int, int)
         */
        ...
        public void onMoved(final RecyclerView recyclerView,
                            final ViewHolder viewHolder, final ViewHolder target, int toPos, int x,
                            int y) {
        ...
        private void onDrawOver(Canvas c, RecyclerView parent, ViewHolder selected,
            ...
            for (int i = 0; i < recoverAnimSize; i++) {
                final ItemTouchHelperExtension.RecoverAnimation anim = recoverAnimationList.get(i);
                final int count = c.save();
//                Callback.sUICallback.onDrawOver(c, parent, anim.mViewHolder.itemView, anim.mX, anim.mY, anim.mActionState, false);
            ...
            if (selected != null) {
                final int count = c.save();
//                Callback.sUICallback.onDrawOver(c, parent, selected.itemView, dX, dY, actionState, true);
